...
class EventListenerUtility:
    ...
    def __init__(
        ...
        self.websocket_url = websocket_url or self._convert_to_websocket_url(
            rpc_url
        )
    ...
    async def _websocket_listener(
        self, contract_address: str, event_obj: Any
    ) -> None:
        ...
        while retry_count < self.max_retries:
            try:
                ...
                self.logger.info(
                    f"Connecting to WebSocket: {self.websocket_url}"
                )
    ...
    async def _log_handler(
        self, handler_context: LogsSubscriptionContext
    ) -> None:
        """
        ...
        try:
            ...
            if hasattr(log_receipt, "get") and callable(log_receipt.get):
                ...
                event_data = {
                    ...
                    "blockNumber": int(
                        str(log_receipt.get("blockNumber", "0x0")), 16
                    ),
                    "data": log_receipt.get("data"),
                    "logIndex": int(
                        str(log_receipt.get("logIndex", "0x0")), 16
                    ),
                ...
                event_data = {
                    ...
                    "logIndex": int(
                        str(getattr(log_receipt, "logIndex", "0x0")), 16
                    ),
                    "topics": getattr(log_receipt, "topics", []),
                    "transactionHash": getattr(
                        log_receipt, "transactionHash", None
                    ),
                    "transactionIndex": int(
